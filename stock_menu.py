...
def delete_stock(stock_list):
    ...
    print("]")
    symbol = input("Which stock symbol do you want to delete: ").upper()
    found = False
    i = 0
    for stock in stock_list:
...
def import_stock_csv(stock_list):
    ...
    filename = input("Please Enter the csv Filename (Example: filename.csv): ")
    for stock in stock_list:
            if stock.symbol == symbol:
                with open (filename, newline='')as stockdata:
                    ...
                    for row in datareader:
                        #The row numbers, or indexes represent the columns in the csv file
